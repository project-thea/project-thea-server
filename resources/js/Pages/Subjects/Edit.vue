...
export default {
    ...
    data() {
        return {
            form: this.$inertia.form({
                ...
                date_of_birth:  this.subject.date_of_birth,
                phone:  this.subject.phone,
                next_of_kin:  this.subject.next_of_kin,
                next_of_kin_phone:  this.subject.next_of_kin_phone,
                id_number:  this.subject.id_number,
                id_type:  this.subject.id_type,
                nationality:  this.subject.nationality,
            }),
        }
    },
    watch: {
    ...
    methods: {
        updateSubject() {
            this.form.patch('/subjects/' + this.subject.id, {
                errorBag: 'updateSubject',
                preserveScroll: true,
                //onSuccess: () => this.form.reset(),
                onError: () => {
                    if (this.form.errors.first_name) {
                        this.form.reset('first_name', this.subject.first_name)
                        this.$refs.first_name.focus()
                    }

                    if (this.form.errors.last_name) {
                        this.form.reset('last_name', this.subject.last_name)
                        this.$refs.last_name.focus()
                    }

                    if (this.form.errors.email) {
                        this.form.reset('id_number', this.subject.email)
                        this.$refs.email.focus()
                    }

                    if (this.form.errors.date_of_birth) {
                        this.form.reset('date_of_birth', this.subject.date_of_birth)
                        this.$refs.date_of_birth.focus()
                    }

                    if (this.form.errors.phone) {
                        this.form.reset('phone', this.subject.phone)
                        this.$refs.phone.focus()
                    }

                    if (this.form.errors.next_of_kin) {
                        this.form.reset('next_of_kin', this.subject.next_of_kin)
                        this.$refs.next_of_kin.focus()
                    }

                    if (this.form.errors.next_of_kin_phone) {
                        this.form.reset('next_of_kin_phone', this.subject.next_of_kin_phone)
                        this.$refs.next_of_kin_phone.focus()
                    }

                    if (this.form.errors.id_number) {
                        this.form.reset('id_number', this.subject.id_number)
                        this.$refs.id_number.focus()
                    }

                    if (this.form.errors.id_type) {
                        this.form.reset('id_type', this.subject.id_type)
                        this.$refs.id_type.focus()
                    }

                    if (this.form.errors.nationality) {
                        this.form.reset('nationality', this.subject.nationality)
